...
if __name__ == "__main__":
    try:
        jobQuery = """
select 
	am.machine_id, am.machine_fqdn, am.platform, am.ip_address, ad.cred_type, ad.username, ad.password, am.backend_port port 
from 
	ai_machine am inner join ai_device_credentials ad on(am.fk_cred_id=ad.cred_id) 
where 
	am.machine_id in(select machine_id from marketplace where lower(vendor)='vmware') and am.active_yn='Y'"""
        jobRet = conn.returnSelectQueryResult(jobQuery)
        if jobRet['result'] == 'success':
            for eachMachine in jobRet['data']:
                cred_type = eachMachine['cred_type'].lower()
                payload = {}
                if cred_type == 'https':
                    password = aes.decrypt(eachMachine['password'].encode(), '@ut0!ntell!'.encode()).decode('utf-8')
                    payload = discoverVCenter(eachMachine['ip_address'], eachMachine['username'], password)
                elif cred_type == 'pam':
                    payload = discoverVCenter(eachMachine['ip_address'], eachMachine['username'])
                else:
                    print('not supported')
                print(payload)
                jobUpdateQuery = ""
                if payload['result'] == 'success':
                    jobUpdateQuery = "update marketplace set payload_stdout='{0}', payload_retcode='{1}' where machine_id={2}".format(
                        json.dumps(payload['data']), '0', eachMachine['machine_id'])
                else:
                    jobUpdateQuery = "update marketplace set payload_stderr='{0}', payload_retcode='{1}' where machine_id={2}".format(
                        json.dumps(payload['data']), '1', eachMachine['machine_id'])
                    print("Unable to fetch the inventory details. Error reason: {0}".format(payload['data']))
                print(jobUpdateQuery)
                upRet = conn.returnInsertResult(jobUpdateQuery)
        else:
            print("No job available to execute.")
    except Exception as e:
        pass
