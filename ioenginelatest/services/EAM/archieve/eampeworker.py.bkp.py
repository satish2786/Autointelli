...
def processEvent(dPayload):
    ...
    try:
        ...
        if not 0 in lPayErr:
            ...
            sQuery = "select * from policyengine where pk_pk_id in(select pe_id from sop where active_yn ='Y' and status='close')"
            dRet = pcon.returnSelectQueryResult(sQuery)
...
try:
    ...
    channel.basic_consume(callback, queue='pe_events', no_ack=True)
