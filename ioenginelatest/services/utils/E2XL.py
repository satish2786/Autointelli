...
try:
    ...
    fqdn = json.load(open('/etc/autointelli/autointelli.conf', 'r'))['fqdn']
...
def export2XLSX(_2DList, pFileName="Alerts"):
    try:
        try:
            ...
            if 1 == 1:
                ...
                xlsxName = pFileName + "_" + str(int(dt.now().timestamp() * 1000000)) + ".xlsx"
                if sSys == "win":
                    xlsxPath = "E:\\" + xlsxName
                else:
                ...
                wb.save(xlsxPath)
                wb.close()
                return {"result": "success", "data": "{0}/downloads/".format(fqdn) + xlsxName}
            else:
