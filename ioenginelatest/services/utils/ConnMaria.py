import mysql.connector as connm
import json
...
tDBUIConn = (cfgData['monitor_maria']['dbip'],
               cfgData['monitor_maria']['dbname'],
               cfgData['monitor_maria']['username'],
               cfgData['monitor_maria']['password'],
               cfgData['monitor_maria']['dbport'])
...
def returnInsertResult(sQuery, sDB="ui"):
    try:
        ...
        conn = connm.connect(host=tParam[0], database=tParam[1], user=tParam[2], password=tParam[3], port=tParam[4])
        cur = conn.cursor()
        cur.execute(sQuery)
        conn.commit()
        iCount = cur.rowcount
