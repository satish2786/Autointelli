...
def sdpCreateTicket(dPayload):
    try:
        # Validate Payload
        lHeaders = ['Title', 'Description', 'Assignment Group']
        lMandate = ['Title', 'Description', 'Assignment Group']
        if validator_many.isPayloadValid(dPayload=dPayload, lHeaders=lHeaders, lMandatory=lMandate):
            # Fetch ITSM settings details
            dRet = fetchITSMSettings()
            if dRet['result'] == 'success':
                sCommunicationType = 'http' #dRet['data'][0]['communication_type']
                sIP = '10.128.97.68' #dRet['data'][0]['ip']
                sPort = '13080' #dRet['data'][0]['port']
                #sTechnicianKey = dRet['data'][0]['technician_key']
                # Call Ticket creation API
                sURL = "{0}://{1}:{2}/SM/9/rest/incidents".format(sCommunicationType, sIP, sPort)
                header = {'content-type': 'application/json'}
                dData = {
        "Incident": {
    "AffectedCI": "srdcb2015stvps",
    "Area": "failure",
    "Assignee": "VIKAS.TRIVEDI",
    "AssignmentGroup": dPayload["Assignment Group"],
    "Category": "incident",
    "Contact": "smint",
    "Descqription": [ dPayload["Description"]
    ],
    "Impact": "2",
    "OpenTime": "2018-07-19T02:15:34+00:00",
    "OpenedBy": "smint",
    "Service": "CI21262",
    "Status": "Open",
    ...
    "Urgency": "3"
                ...
                objRet = req.post(url=sURL, json=dData, auth=HTTPBasicAuth("hpsm.admin","Dev@1234"), headers=header)
                print(dData)
                print(objRet.status_code)
                if objRet.status_code == 200:
                    dOutput = json.loads(objRet.text)
                    sTicketID = dOutput['Incident']['IncidentID']
                    # make entry - ai_ticket_details
                    iAlertID = int(dPayload['Alert_id'].strip('AL'))
                    iTktID = sTicketID
                    dEpoch = aitime.getcurrentutcepoch()
                    sQuery = "insert into ai_ticket_details(fk_alert_id, ticket_no, ticket_status, created_date) values(%d,'%s','Open',%d)" %(iAlertID, iTktID, dEpoch)
                    print(sQuery)
    ...
    try:
        ...
        if validator_many.isPayloadValid(dPayload=dPayload, lHeaders=lHeaders, lMandatory=lMandate):
            ...
            if dRet['result'] == 'success':
                ...
                #sTechnicianKey = dRet['data'][0]['technician_key']
                # Call Ticket creation API
                sURL = "{0}://{1}:{2}/SM/9/rest/incidents/{3}".format(sCommunicationType, sIP, sPort, dPayload["Ticket ID"])
                header = {'content-type': 'application/json'}
                dData = {
    "Incident": {
        "Status": dPayload["Status"]
    }
}
                objRet = req.post(url=sURL, json=dData, auth=HTTPBasicAuth("hpsm.admin", "Dev@1234"), headers=header)
                if objRet.status_code == 200:
...
def sdpUpdateTicketGroup(dPayload):
    try:
        # Validate Payload
        lHeaders = ["Ticket ID", "Assignment Group"]
        lMandate = ["Ticket ID", "Assignment Group"]
        if validator_many.isPayloadValid(dPayload=dPayload, lHeaders=lHeaders, lMandatory=lMandate):
            # Fetch ITSM settings details
            dRet = fetchITSMSettings()
            if dRet['result'] == 'success':
