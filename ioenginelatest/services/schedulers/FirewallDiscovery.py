...
def insertin2DB(id, ip, ints):
    try:
        # Pull Existing
        sExist = {}
        sQuery = "select firewall_ip, int_id, int_name from firewall_object_inventory where firewall_ip='{0}' and active_yn='Y'".format(ip)
        sRet = pcon.returnSelectQueryResultAs2DList(sQuery)
        if sRet['result'] == 'success':
            sExist = set([tuple(i) for i in sRet['data'][1:]])

        iQuery = "insert into firewall_object_inventory(fk_hypervisor_id, firewall_ip, int_id, int_name, created_date, created_by, active_yn) " \
...
if __name__ == "__main__":
    try:
        ...
        sQuery = """
                        ...
                        where 
            	            lower(trim(h.hypervisor_type))='firewall' and h.active_yn='Y' """
