...
def insertin2DB(id, ip, ints):
    try:
        # Pull Existing
        sExist = {}
        sQuery = "select router_ip, int_id, int_name from router_object_inventory where router_ip='{0}' and active_yn='Y'".format(ip)
        sRet = pcon.returnSelectQueryResultAs2DList(sQuery)
        if sRet['result'] == 'success':
            sExist = set([tuple(i) for i in sRet['data'][1:]])

        iQuery = "insert into router_object_inventory(fk_hypervisor_id, router_ip, int_id, int_name, created_date, created_by, active_yn) " \
...
if __name__ == "__main__":
    try:
        ...
        sQuery = """
                        ...
                        where 
            	            lower(trim(h.hypervisor_type))='router' and h.active_yn='Y' """
