...
def discoverSwitchInterfaces(ip, comm_string):
    try:
        finalIFs = []
        t = dt.utcnow()
...
def insertin2DB(id, ip, ints):
    try:
        # Pull Existing
        sExist = {}
        sQuery = "select switch_ip, int_id, int_name from switch_object_inventory where switch_ip='{0}' and active_yn='Y'".format(ip)
        sRet = pcon.returnSelectQueryResultAs2DList(sQuery)
        if sRet['result'] == 'success':
            sExist = set([tuple(i) for i in sRet['data'][1:]])

        iQuery = "insert into switch_object_inventory(fk_hypervisor_id, switch_ip, int_id, int_name, created_date, created_by, active_yn) " \
...
if __name__ == "__main__":
    try:
        ...
        sQuery = """
                        ...
                        where 
            	            lower(trim(h.hypervisor_type))='switch' and h.active_yn='Y' """
        ...
        if dRet["result"] == "success":
            for eachKVM in dRet["data"]:
                try:
                    ...
                    ret = discoverSwitchInterfaces(ip, comm_string)
