...
class multilang:
        ...
        try:
            if 'target' in list(self.global_vars['VARS'].keys()):
                ...
                if self.HOSTNAME == "localhost":
                    return True
                sQuery = """select 
                                am.machine_fqdn, am.platform, am.ip_address, ad.cred_type, ad.username, ad.password, am.backend_port port 
                            from 
                                ai_machine am inner join ai_device_credentials ad on(am.fk_cred_id=ad.cred_id) 
                            where 
                                am.machine_fqdn='{0}' or am.ip_address='{1}' and am.active_yn='Y'""".format(self.HOSTNAME, self.HOSTNAME)
                ...
                if dRet['result'] == 'success':
                    if dRet['data'][0]['cred_type'].lower() == 'pam':
                        ...
                        if ret["result"] == "success":
                            ...
                            self.PASSWORD = ret["data"]['password']
        ...
        try:
            ...
            for K in list(tempobj.keys()):
                ...
                if tempobj[K].strip() != "" and len(tempobj[K].strip().splitlines()) > 1:
                    ...
                    PreS += "{0}=\"{1}\";{2}".format(K, tempobj[K].replace('"', '\\"'), self.NEWLINE)
            tmpScript = PreS + tmpScript + self.NEWLINE
            logINFO("Generated Prefix: {0}".format(PreS))
    ...
    def execute_script_powershell(self):
        ...
        # added by dinesh on 20-AUG-2021
        # This will handle the foreground execution on windows system
        foregroundFlag, foregroundScript = 'N', ""
        if 'foreground' in tempobj:
            foregroundFlag = tempobj['foreground']
            ...
            try:
                ...
                try:
                    if foregroundFlag == 'Y':
                        foregroundScript = """
# Auto Generate vars
$randomName = Get-Date -Format "ddMMyyyyhhmmssfff"
$targetTmpDir = $env:TEMP
$fileFullPath = "$targetTmpDir/$randomName.ps1"

# Create File
New-Item -Path $fileFullPath -ItemType File -Value @'
{0}
'@  -Force
...
$Action= New-ScheduledTaskAction -Execute "PowerShell.exe" -Argument "$fileFullPath"
Register-ScheduledTask -TaskName $randomName  -User $User -Action $Action -RunLevel Highest –Force #-Trigger $Trigger

# Start Scheduler
Start-ScheduledTask -TaskName $randomName

# Remove item
Unregister-ScheduledTask -TaskName $randomName -Confirm:$false
Remove-Item -Path $fileFullPath -Force""".format(tmpScript)
                        tmpScript = foregroundScript
                    print(tmpScript)
